...
export function SneakerCard({ sneaker, birthday }: SneakerCardProps) {
    ...
    return (
        <Link to="/sneakers/$id" params={{ id: sneaker._id }} className={cn("min-w-60 block relative p-2 pr-6 bg-secondary rounded-2xl hover:shadow-2xl hover:shadow-primary/5 group ring ring-border/75 hover:border-white/20 overflow-hidden transition-shadow duration-300 z-0", !birthday ? "w-full" : "shrink-0")}>
                    ...
                    <div className="flex items-center gap-1.5 text-zinc-400 group-hover:text-zinc-300 transition-colors">
                        {!birthday ? (
                            <>
                                ...
                                <span className="text-primary-100 group-hover:text-primary-300 text-sm font-semibold opacity-75 group-hover:opacity-100 truncate transition">
